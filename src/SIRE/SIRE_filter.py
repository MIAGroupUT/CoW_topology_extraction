...
from src.SIRE.transforms import (
    MakeMultiscaleDataBatch,
    MakeMultiscaleData,
    CreateSIREGrid,
)
...
class ProcessSIRE3DImage:
    ...
    def process_points(self):
        ...
        for i, points in tqdm(enumerate(torch.split(gridpoints, 500))):
            ...
            for j in range(points.shape[0]):
                output = self.process_pred(
                    pred_batch[
                        j * self.params["nverts"] : (j + 1) * self.params["nverts"], :
                    ],
                    sphere,
                )
                for key in output:
                    results[key].append(output[key])
                index_vox = np.unravel_index(
                    (500 * i) + j, self.img["vectorfield"].shape[1:]
                )
                o = torch.tensor(
                    [output["entropy"], output["max_activation"], output["max_scale"]]
                )
                self.img["vectorfield"][:, index_vox[0], index_vox[1], index_vox[2]] = (
                    torch.cat([output["d1"], output["d2"], o])
                )
    ...
    def process_pred(self, pred, sphere):
        ...
        if torch.max(pred) > 0:
            max_scale = self.params["raylengths"][
                torch.argmax(torch.max(pred, dim=0)[0])
            ]
        ...
        if activations.sum() > 0:
            ...
            binsizes = np.array(
                [diffs[0]] + (0.5 * diffs[:-1] + 0.5 * diffs[1:]).tolist() + [diffs[-1]]
            )
            avg_scale = np.average(
                np.array(self.params["raylengths"]),
    ...
    def get_transform(self):
        loader = LoadImaged(keys=["img"], image_only=False, allow_missing_keys=True)
        ...
        transform = Compose(
            [
                ...
                ScaleIntensityRangePercentilesd(
                    keys=["img"], lower=1, upper=99, b_min=0.0, b_max=1.0, clip=True
                ),
                MakeMultiscaleData(
                    self.params["raylengths"],
                    GEMGCN=False,
                    measure=self.params["measure"],
                    subdivisions=self.exp_params["TransformParams"]["subdivisions"],
                    npoints=self.exp_params["TransformParams"]["npoints"],
                ),  # gem=false because of unequal batch sizes at end of grid
            ]
        )
        return transform

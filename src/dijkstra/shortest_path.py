from src.dijkstra.transforms import (
    GetStartEndPoints,
    InterpolateImage,
    get_cos_similarity,
)
...
class ExtractPaths:
    ...
    def get_transform(self):
        ...
        if self.params["vis"]:
            ...
            transform += [
                loader,
                CreateSIREGrid(
                    "bifs", crop_margin=5, grid_resolution=0.3, grid_key="MIP_grid"
                ),
            ]
        transform += [
            CreateSIREGrid(
                "bifs", crop_margin=5, grid_resolution=self.params["grid_resolution"]
            ),
    ...
    def get_cost_function(self, data):
        ...
        cossim = np.clip(get_cos_similarity(data["vectorfield"]), -1, 1)
        c_orient = 1 - ((cossim + 1) / 2)
        return (
            # self.params["c_img"] * c_img
            # + self.params["c_entr"] * c_entr
            self.params["c_orient"]
            * c_orient
        )
    ...
    def make_MIP_plot(self, data, paths):
        ...
        for j, arti in enumerate(list(paths)):
            ...
            p_vox = transform_points(
                paths[arti], np.linalg.inv(sample["MIP_grid_meta"]["affine"])
            )
            plt.plot(
                p_vox[:, 0],
                p_vox[:, 1],
        ...
        bifs_MIP = transform_points(
            sample["BoIs"]["world"], np.linalg.inv(sample["MIP_grid_meta"]["affine"])
        )
        plt.plot(
            bifs_MIP[:, 0],
            bifs_MIP[:, 1],
        ...
        plt.savefig(os.path.join("../results/paths/", data["ID"] + "_paths.png"))
        plt.close()
    ...
    def __call__(self, data):
        ...
        for arti in list(data["seedpoints"]):
            ...
            clines["world"][arti] = transform_points(path, data["grid_meta"]["affine"])
            clines["grid_coords"][arti] = path

        if self.params["vis"]:
            fig = self.make_MIP_plot(data, clines["world"])
            return clines, fig
        return clines
